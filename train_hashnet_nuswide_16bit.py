...
RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results', 'NUSWIDE_Top21', 'HashNet', 'ResNet50', f'{BIT}bits')
...
class HashNet_ResNet50(nn.Module):
    def __init__(self, bit=16, pretrained_path=None):
...
def train_hashnet(bit, train_loader, query_loader, db_loader, eval_indices=None, resume_from=None):
    print(f"\n>>> Start Training HashNet-ResNet50 ({bit} bits) for {EPOCHS} epochs on NUS-WIDE <<<")
    print(f">>> Using GPU: {DEVICE}")

    model = HashNet_ResNet50(bit=bit, pretrained_path=PRETRAINED_PATH).to(DEVICE)
...
def main():
    ...
    train_hashnet(BIT, train_loader, query_loader, db_loader, eval_indices=top21_indices, resume_from=args.resume)
