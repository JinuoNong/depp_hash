...
RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results', 'MSCOCO', 'CSQ', 'ResNet50', f'{BIT}bits')
...
class CSQ_ResNet50(nn.Module):
    def __init__(self, bit=32, num_classes=80, pretrained=True):
...
def train_csq(bit, train_loader, query_loader, db_loader, resume_from=None):
    print(f"\n>>> Start Training CSQ-ResNet50 ({bit} bits) for {EPOCHS} epochs on MSCOCO2014 <<<")
    print(f">>> Using GPU: {DEVICE}")
    if torch.cuda.is_available():
        print(f">>> GPU Name: {torch.cuda.get_device_name(DEVICE)}")
        print(f">>> GPU Memory: {torch.cuda.get_device_properties(DEVICE).total_memory / 1024**3:.2f} GB")

    model = CSQ_ResNet50(bit=bit, num_classes=NUM_CLASSES).to(DEVICE)
