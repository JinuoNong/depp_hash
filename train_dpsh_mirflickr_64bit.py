...
parser.add_argument('--gpu', type=int, default=None, help='GPU device ID (default: use visible GPUs)')
...
from src.models.losses import calc_dpsh_loss
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results', 'MIRFlickr25K', 'DPSH', 'ResNet50', f'{BIT}bits')
...
def train_dpsh(bit, train_loader, query_loader, db_loader, resume_from=None):
    print(f"\n>>> Start Training DPSH-ResNet50 ({bit} bits) for {EPOCHS} epochs on MIRFlickr25K <<<")
    print(f">>> Using GPU: {DEVICE}")
    print(f">>> Evaluation Protocol: Top-21 + mAP@{MAP_TOPK}")
...
def main():
    ...
    print(f"Evaluation: Top-21 + mAP@{MAP_TOPK}")

    train_dpsh(BIT, train_loader, query_loader, db_loader, resume_from=args.resume)
